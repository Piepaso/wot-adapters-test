    ...
    public void onIncomingPhysicalAction(PhysicalAssetActionWldtEvent<?> physicalAssetActionWldtEvent) {
        try{
            ...
            if(physicalAssetActionWldtEvent != null) {
                ...
                System.out.println("["+ getId() +"] -> Received Action Request: " + physicalAssetActionWldtEvent.getActionKey()
                        + " with Body: " + physicalAssetActionWldtEvent.getBody());
            }
            else
                System.err.println("["+ getId() +"] -> Wrong Action Received !");
    ...
    private Runnable publishPhysicalAssetDescription(){
        return () -> {
            try {
                ...
                System.out.println("["+ getId() +"] -> Sleeping before Publishing Physical Asset Description ...");

                //Emulate a Startup delay of 5 seconds to emulate device startup
                Thread.sleep(5000);

                System.out.println("["+ getId() +"] -> Publishing Physical Asset Description ...");

                //Create an empty PAD
                PhysicalAssetDescription pad = new PhysicalAssetDescription();

                //Add a new Property associated to the target PAD with a key and a default value
                PhysicalAssetProperty<TemperaturePropertyObject> temperatureProperty = new PhysicalAssetProperty<TemperaturePropertyObject>(
                    GlobalKeywords.TEMPERATURE_PROPERTY_KEY,
                    new TemperaturePropertyObject(0.0, 0, List.of(false, false, false))
                );
    ...
    private Runnable deviceEmulation(){
        return () -> {
            try {
                ...
                System.out.println("["+ getId() +"] -> Sleeping before Starting Physical Device Emulation ...");

                //Sleep 5 seconds to emulate device startup
                Thread.sleep(10000);

                System.out.println("["+ getId() +"] -> Starting Physical Device Emulation ...");
                ...
                for(int i = 0; i < GlobalKeywords.MESSAGE_UPDATE_NUMBER; i++){
                    ...
                    //Update the
                    TemperaturePropertyObject randomTemperature = new TemperaturePropertyObject(
                            r.nextDouble() * (GlobalKeywords.TEMPERATURE_MAX_VALUE - GlobalKeywords.TEMPERATURE_MIN_VALUE) + GlobalKeywords.TEMPERATURE_MIN_VALUE,
                            i,
                            List.of(r.nextBoolean(), r.nextBoolean(), r.nextBoolean())
                    );

                    //Create a new event to notify the variation of a Physical Property
                    PhysicalAssetPropertyWldtEvent<TemperaturePropertyObject> newPhysicalPropertyEvent = new PhysicalAssetPropertyWldtEvent<>(GlobalKeywords.TEMPERATURE_PROPERTY_KEY, randomTemperature);
